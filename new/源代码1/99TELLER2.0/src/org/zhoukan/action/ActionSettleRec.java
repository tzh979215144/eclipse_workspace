...
public class ActionSettleRec extends Action {
	...
	protected String excuse(Map<String, String> map,
		...
		SettleRecBean settleRecBean = this.doAfter(esb2TellerXML);

		request.setAttribute("settleRecBean", settleRecBean);

		try {
			request.getRequestDispatcher("/teller/settleRec.jsp").forward(
					request, response);
	...
	private SettleRecBean doAfter(String xml) {
		// TODO Auto-generated method stub
		...
		settleRecBean.setRecieveMess(praseXML.getLocalHeadRE_CODEText());

		try {
			BeanUtils.populate(streamBean, mao);
			BeanUtils.populate(settleRecBean, map);
		...
		return settleRecBean;
	}

	private String initRegister(Map<String, String> map) {
		// TODO Auto-generated method stub
		PraseXML praseXML = new PraseXML(new File(
				"E:\\Workspaces\\MyEclipse 8.5\\TELLER2.0\\src\\Send.xml"));

		praseXML.setInMessage(map);

		praseXML.setLocalHeadTRAN_CODEText("CL07");

		praseXML.setLocalHeadTRAN_NAMEText("回收结算");

		praseXML.setLocalHeadTRAN_SEQText(""
				+ (new GetNOSP().getNO("front_tran_no")));
		...
		praseXML.setBodySVR_IDText("CL07");
