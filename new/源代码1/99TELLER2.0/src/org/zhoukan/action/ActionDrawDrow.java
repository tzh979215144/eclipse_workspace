...
public class ActionDrawDrow extends Action{
	...
	protected String excuse(Map<String, String> map,
		...
		DrawDrowBean drawdrowBean = this.doAfter(esb2TellerXML);
		
		
		
		request.setAttribute("DrawDrowBean", drawdrowBean);
		
		try {
			request.getRequestDispatcher("/teller/drawDown.jsp").forward(request, response);
	...
	public String initOpen(Map<String, String> map){
		...
		praseXML.setLocalHeadTRAN_CODEText("CL02");
		
		praseXML.setLocalHeadTRAN_NAMEText("贷款发放");
		...
		praseXML.setBodySVR_IDText("CL02");
	...
	public DrawDrowBean doAfter(String xml){
		PraseXML praseXML = new PraseXML(xml);
		
		DrawDrowBean drawDrowBean = new DrawDrowBean();
		...
		drawDrowBean.setRecieveMess(praseXML.getLocalHeadRE_CODEText());
		
		try {
			BeanUtils.populate(streamBean, mao);
			BeanUtils.populate(drawDrowBean, map);
