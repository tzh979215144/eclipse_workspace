...
public class ActionReceipt extends Action{
	...
	protected String excuse(Map<String, String> map,
		...
		ReceiptBean receiptBean = this.doAfter(esb2TellerXML);
		
		
		
		request.setAttribute("receiptBean", receiptBean);
		
		try {
			request.getRequestDispatcher("/teller/receipt.jsp").forward(request, response);
	...
	public String initRegister(Map<String, String> map){
		...
		praseXML.setLocalHeadTRAN_CODEText("CL61");
		
		praseXML.setLocalHeadTRAN_NAMEText("贷款回收");
		...
		praseXML.setBodySVR_IDText("CL61");
	...
	public ReceiptBean doAfter(String xml){
		...
		receiptBean.setRecieveMess(praseXML.getLocalHeadRE_CODEText());
		
		try {
			BeanUtils.populate(streamBean, mao);
			BeanUtils.populate(receiptBean, map);
		} catch (IllegalAccessException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (InvocationTargetException e) {
