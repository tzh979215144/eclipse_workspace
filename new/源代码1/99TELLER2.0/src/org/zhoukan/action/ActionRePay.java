...
public class ActionRePay extends Action{
	...
	protected String excuse(Map<String, String> map,
		...
		RepayBean repayBean = this.doAfter(esb2TellerXML);
		
		request.setAttribute("repayBean", repayBean);
		
		try {
			request.getRequestDispatcher("/teller/printRepay.jsp").forward(request, response);
	...
	public String initRegister(Map<String, String> map){
		...
		praseXML.setLocalHeadTRAN_CODEText("SB01");
		
		praseXML.setLocalHeadTRAN_NAMEText("还款计划");
		...
		praseXML.setBodySVR_IDText("SB01");
	...
	public RepayBean doAfter(String xml){
		...
		repayBean.setRecieveMess(praseXML.getLocalHeadRE_CODEText());
		
		try {
			BeanUtils.populate(streamBean, mao);
			BeanUtils.populate(repayBean, map);
		} catch (IllegalAccessException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (InvocationTargetException e) {
