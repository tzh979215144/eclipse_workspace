...
import org.zhoukan.bean.settlePayBean;
...
public class ActionSettlePay extends Action{
	...
	protected String excuse(Map<String, String> map,
		...
		settlePayBean settlePayBean = this.doAfter(esb2TellerXML);
	...
	public String initOpen(Map<String, String> map){
		...
		praseXML.setLocalHeadTRAN_CODEText("CL03");
		
		praseXML.setLocalHeadTRAN_NAMEText("发放结算");
		...
		praseXML.setBodySVR_IDText("CL03");
	...
	public settlePayBean doAfter(String xml){
		PraseXML praseXML = new PraseXML(xml);
		
		settlePayBean settlePayBean = new settlePayBean();
		...
		settlePayBean.setRecieveMess(praseXML.getLocalHeadRE_CODEText());
