...
public class WithDrawingSP {
	...
	public String update(Fm_acctBean bean){
		...
		String n = "-1";
		try {
			cs = conn.prepareCall("{call SP_WITH_DRAWING(?,?,?)}");
			cs.setString(1, bean.getBASE_ACCT_NO());
			cs.setString(2, bean.getWITHDRAWING_AMT());
	...
	public static void main(String[] args) {
		...
		WithDrawingSP withDrwingSP= new WithDrawingSP();
		fm_acctBean.setBASE_ACCT_NO("141020801033");
		fm_acctBean.setWITHDRAWING_AMT("12000");
		String out = withDrwingSP.update(fm_acctBean);
